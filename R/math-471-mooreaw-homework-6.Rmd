...
**c)** Conditions for inference

```{r}
qq_r <- lst(
  y1982 = d611 %>% filter(year == "1982") %>% pull(pcb),
  y1996 = d611 %>% filter(year == "1996") %>% pull(pcb)
) %>%
  map_dbl(function(x) {
    qq <- qqnorm(x, plot.it = FALSE)
    cor(qq$x, qq$y)
...
ggplot(d611, aes(sample = pcb)) +
  ...
  labs(
    x = "Normal Quantiles",
    y = "PCB Content",
    caption = str_glue("1982, *r* = {qq_r[1]}; 1996, *r* = {qq_r[2]}")
  )

ggplot(d611, aes(x = pcb)) +
  geom_histogram(color = "white") +
  facet_wrap(~year) +
  labs(x = "PCB Content", y = "Frequency")
```
